...
if(acesso().filtro != ''){
    socket.emit('evento-filtro', {config:config, filtro: acesso().filtro})
}else{
    socket.emit('rodar', {config: config})
}

socket.on('evento-filtro', function(valor){
    var dados 
    if(acesso().filtro != ''){
        dados = valor.dadosFiltro
        console.log(dados)
    }else{
        return
    }
...
socket.on('rodar', function(valor){
    var dados 
    if(acesso().filtro != ''){
        return
    }else{
        dados = valor.dadosReal
    }
    
   renderDados(dados)
})

socket.on('filtro-todos', function(valor){
    var dados 
    if(acesso().filtro != ''){
        dados = valor.dadosFiltroTotal
        console.log(dados)
    }else{
        return
    }
...
function renderDados(dados){
    ...
    Object.entries(dados.solicitados).forEach(([key, val]) => {
        //ICONE DE URGENCIA
        let urgente = ''
    ...
    Object.entries(dados.agendados).forEach(([key, val]) => {
        //ICONE DE URGENCIA
        let urgente = ''
        if (val.tipo_atendimento == 'U') {
            urgente = `<em class="icon ni ni-plus-medi-fill text-danger"></em>`
    ...
    $('#Atendimento').empty();
    //
    Object.entries(dados.atendimento).forEach(([key, val]) => {
        //ICONE DE URGENCIA
        let urgente = ''
        if (val.tipo_atendimento == 'U') {
            urgente = `<em class="icon ni ni-plus-medi-fill text-danger"></em>`
    ...
    $('#posExame').empty();
    Object.entries(dados.pos_exame).forEach(([key, val]) => {
        //ICONE DE URGENCIA
        let urgente = ''
        if (val.tipo_atendimento == 'U') {
            urgente = `<em class="icon ni ni-plus-medi-fill text-danger"></em>`
    ...
    $('#finalizados').empty();
    Object.entries(dados.finalizados).forEach(([key, val]) => {
        //ICONE DE URGENCIA
        let urgente = ''
        if (val.tipo_atendimento == 'U') {
            urgente = `<em class="icon ni ni-plus-medi-fill text-danger"></em>`
    ...
    $("#SolicitadosCount").text(`${dados.count.total_solicitatos}`)
    $("#AgendadosCount").text(`${dados.count.total_agendados}`)
    $("#AtendimentoCount").text(`${dados.count.total_atendimento}`)
    $("#PosExameCount").text(`${dados.count.total_pos_exame}`)
    $("#FinalizadosCount").text(`${dados.count.total_finalizados}`)
